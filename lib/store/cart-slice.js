...
const cartSlice = createSlice({
  ...
  reducers: {
    ...
    changeModelQuantityCalc(state, action) {
      ...
      const existingItems = state.order1.items.filter(
      ...
      if (parseInt(modelName.quantity) == 0) {
        state.order1.items = state.order1.items.filter(
          (item) => item.model !== modelName.model
        );
        state.order1.totalQuantityModel =
          state.order1.totalQuantityModel.filter(
            (item) => item.model !== modelName.model
          );
      } else {
        if (test > 0) {
    ...
    changeModelQuantity(state, action) {
      const modelName = action.payload;
      const existingModel = state.order1.totalQuantityModel.find(
        (item) => item.model === modelName.model
      );
      if (existingModel) {
    ...
    addItemToCart(state, action) {
      ...
      const existingItem = state.order1.items.find(
      ...
      if (!existingModel) {
        state.order1.totalQuantityModel.push({
          model: newItem.model,
          modelQuantity: 1,
        });
      } else {
