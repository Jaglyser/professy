...
const cartSlice = createSlice({
  ...
  reducers: {
    changeModelQuantity(state, action) {
      ...
      const test = parseInt(modelName.quantity) - parseInt(existingModel.modelQuantity)
      const change = Math.abs(parseInt(modelName.quantity) - parseInt(existingModel.modelQuantity))
      const changeLeft = Math.abs(parseInt(modelName.quantity) - parseInt(existingModel.modelQuantity))
      console.log(change);
      if(test > 0) {
        for (let i = 0; i < existingItems.length; i++) {
          const amountToAdd = Math.ceil((parseInt(change) * ((existingItems[i].quantity)/existingModel.modelQuantity)))
        ...
        for (let i = 0; i < existingItems.length; i++) {
          const amountToRemove = Math.ceil((parseInt(change) * ((existingItems[i].quantity)/existingModel.modelQuantity)))
      ...
      if (existingModel) {
        if (modelName.quantity.trim().length == 0) {
          state.totalQuantityModel = state.totalQuantityModel.filter(item => item.model !== modelName.model)
        } else {
